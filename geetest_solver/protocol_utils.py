...
def _get_string_by_indexes(indexes: list, s: str) -> str:
    """lot_number 文字列から指定範囲を切り出して `.` でつなげます。"""
    parts = []
...
def parse_abo_pair(key: str, value: str, lot_number: str) -> dict:
    """`a.b.c` みたいなドット付きキー文字列からネスト辞書を作ります。
...
def generate_pow(lot_number: str, captcha_id: str, *,
                 ...
                 extra: str = "") -> dict:
    """指定難易度を満たすハッシュが出るまでランダム値を回し続けます。

    判定条件 (JS とおそろい):先頭 `bits//4` 桁が `0` で、次の1桁がしきい値以下になればOK。
    """
    if hashfunc not in _HASH_FN:
        raise ValueError(f"未対応の PoW ハッシュです: {hashfunc}")
